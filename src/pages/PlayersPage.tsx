...
// Make sure this path matches where you saved your data file
...
export default function PlayersPage() {
  ...
  // Helper to safely get stats based on the toggle state passed in
  const getStat = (p: WikiPlayer, statKey: string, useMax: boolean) => {
    if (!p.stats) return "-";
    const levelKey = useMax ? "max" : "lvl1";
    
    // Check both capitalized (Serve) and lowercase (serve) just in case
  ...
  // Handler to open modal and sync the toggle
